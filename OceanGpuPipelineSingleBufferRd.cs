...
public partial class OceanGpuPipelineSingleBufferRd : Node
{
    // -----------------------------
    // User settings
    // -----------------------------
    ...
    private Texture2Drd _texDxDisplay;
    private Texture2Drd _texDzDisplay;
    ...
    private Rid _setE_Dx;
    private Rid _setE_Dz;
    ...
    public override void _Process(double delta)
    {
        ...
        if (_titleLabel != null)
            _titleLabel.Text = $"Ocean A→E (GPU RD, single-buffer)  t={_timeSec:0.00}  λ={ChoppyLambda:0.00}";
    ...
    private void RenderThreadInit()
    {
        try
        {
            ...
            _rdReady = true;
            GD.Print("Ocean RD pipeline initialized (single-buffered Texture2Drd preview).");
        }
        catch (Exception e)
        {
    ...
    private void RenderThreadStep()
    {
        ...
        try
        {
            ...
            // dy / dx / dz FFT + inversion
            RunFftAndInvertForComponent(cl, _setD_Dy, _setE_Dy, groups16);
            RunFftAndInvertForComponent(cl, _setD_Dx, _setE_Dx, groups16);
            RunFftAndInvertForComponent(cl, _setD_Dz, _setE_Dz, groups16);
    ...
    {
        // Start with fresh data in ping0 from Appendix B.
        int pingpong = 0;
        ...
        // Correct final source (fixes center seam).
    ...
    private void BuildPreviewUi()
    {
        ...
        {
            Text = "Ocean A→E (GPU RD, single-buffer)",
...
void fragment() {
    ...
    public void BindPreviewTextures()
    {
        ...
        _texDxDisplay = new Texture2Drd { TextureRdRid = _dispDx };
        _texDzDisplay = new Texture2Drd { TextureRdRid = _dispDz };
    ...
    private void ApplyPreviewTextures()
    {
        ...
        _spriteChoppy.Texture = _texDyDisplay;

        _matHeight?.SetShaderParameter("dy_tex", _texDyDisplay);

        _matChoppy?.SetShaderParameter("dy_tex", _texDyDisplay);
        _matChoppy?.SetShaderParameter("dx_tex", _texDxDisplay);
        _matChoppy?.SetShaderParameter("dz_tex", _texDzDisplay);
    }
    ...
    private void CreateAppendixTextures()
    {
        ...
        // Final display textures (compute writes + canvas samples)
        _dispDy = CreateStorageTextureRGBA16F(N, N, sampled: true);
        _dispDx = CreateStorageTextureRGBA16F(N, N, sampled: true);
        _dispDz = CreateStorageTextureRGBA16F(N, N, sampled: true);
        ...
        TrackRid(_dispDy); TrackRid(_dispDx); TrackRid(_dispDz);
    }
    ...
    private void CreateUniformSets()
    {
        ...
        _setE_Dx = CreateSetE(_dispDx, _dxPing0, _dxPing1);
        _setE_Dz = CreateSetE(_dispDz, _dzPing0, _dzPing1);
    }
